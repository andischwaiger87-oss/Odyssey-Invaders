...
export class CollisionSystem implements System {
  update(entities: Entity[], engine: Engine, delta: number): void {
    for (const entity of entities) {
      if (engine.em.hasComponent(entity, "Health")) {
        ...
        if (health.invulnerableTimer > 0) health.invulnerableTimer -= delta;
      }
    }
    ...
    for (let i = 0; i < collidables.length; i++) {
      for (let j = i + 1; j < collidables.length; j++) {
        ...
        if (!engine.em.hasComponent(e1, "Collider") || !engine.em.hasComponent(e2, "Collider")) continue;
  ...
  private dropBalancedPowerUp(engine: Engine, x: number, y: number) {
    ...
    let faction = "POWERUP_TRI"; let color = "#00ffcc";
    if (rand < 0.35) { faction = "POWERUP_SHIELD"; color = "#38bdf8"; }
    else if (rand < 0.55) { faction = "POWERUP_LIFE"; color = "#f43f5e"; }
