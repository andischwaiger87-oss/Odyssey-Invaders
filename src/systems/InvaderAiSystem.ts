...
export class InvaderAiSystem implements System {
    ...
    for (const invader of invaders) {
      ...
      switch (render.type) {
        case "cube": // Primitiv-Horde (Akt I)
          ...
          // KORREKTUR: Knochenschiffe schießen jetzt mit unübersehbarem Elektro-Pink (#ff3399)
        ...
        case "predator": // Urzeit-Raubtiere (Akt I)
          pos.x += Math.cos(this.waveTimer * 5) * 110 * delta;
          pos.y += (35 + engine.currentLevel * 3) * delta;
          
          // KORREKTUR: Raubtiere feuern giftiges Neon-Orange (#ff6600)
        ...
        case "alien": // Plasma-Biomasse (Akt II & V)
          ...
          // KORREKTUR: Aliens schießen strahlendes Cyber-Lila (#d000ff)
          if (this.globalShootCooldown <= 0 && playerPos && Math.random() < 0.04) {
            const trackVector = (playerPos.x - pos.x) * 0.7;
  ...
  private fireEnemyCapsuleLaser(engine: Engine, x: number, y: number, color: string, vx: number): void {
    ...
    engine.em.addComponent(laser, new Renderable(color, 16, "laser")); // '16' definiert die Strichlänge
